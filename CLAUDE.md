...
- `apps/gba` is fully playable end to end: real mGBA core over JNI (`vendor/mgba` git submodule — run `git submodule update --init` after cloning), battery saves force-flushed on backgrounding, nine savestate slots with thumbnails, slot-0 auto-save/resume, and "Open with" from file managers (`useOpenedRom` + `importRomFromUri`; `apps/nds`/`threeds` still need their own `app.json` intent filters — the JS half is shared).
...
Still to build: 3DS SMDH icons (needs ExeFS decryption), gamepad customization, and user-selectable launcher icons (planned in `docs/app_icon_plan.md`).
