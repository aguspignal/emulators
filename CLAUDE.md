...
- Anything that differs between apps flows through `AppConfig` (defined in `packages/ui/src/config.tsx`): app name, console specs, the `core` object, the native `EmulatorView` component, and the bundled `coverIndexes` (so each app ships only its own consoles' lookup tables). Each app's `App.tsx` builds its config and renders the shared `<AppRoot />`. Shared screens read it via `useAppConfig()` — never import app code from `packages/*`.
