...
- `apps/threeds` vendors real Azahar 2126.0 (submodule with 36 nested submodules — clone with `git submodule update --init --recursive`) with the native engine and full Kotlin contract implemented; on-device verification pending. Its minSdk is 29 (Azahar's floor), unlike the other apps' 24. Still missing: intent filters, the `sharedFiles` bridge, `eas init`.
