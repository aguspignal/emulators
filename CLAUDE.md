...
Scaffolding is complete and typechecks. `apps/gba` has the real mGBA core integrated over JNI (git submodule at `apps/gba/modules/mgba-core/android/vendor/mgba`, pinned to 0.10.5 — run `git submodule update --init` after cloning). The ROM library is built: SQLite-backed Home screen with import (picker → MD5 dedup → copy into `roms/`), favorite/delete, and boot-on-tap into the Emulator screen. "Add ROM" offers both a single file and a whole folder — `pickAndImportFolder` walks the granted tree to any depth, runs the same per-file import behind a blocking progress overlay, and reports added/duplicate/skipped/failed counts in one alert (`docs/folder_import_plan.md`). Games are playable: the shared on-screen gamepad (`packages/ui/src/components/gamepad/`) drives `core.setButton` with real multi-touch and diagonals, plus a pause menu (Resume / Save state / Load state / Reset / Exit). The emulator screen rotates with the phone — landscape floats the pad over a full-bleed game, portrait puts the game on top with the pad in a band below. Saves are done: battery saves persist and are force-flushed on backgrounding, nine savestate slots with thumbnails are picked from the pause menu, and slot 0 auto-saves on exit/background so reopening a game resumes it. Cover art is done: the Home screen is a poster grid fed by box art from the libretro thumbnail CDN, matched offline against per-app MD5→name indexes generated by `scripts/build-cover-index.mjs`, with DS cartridge banner icons as a fallback and a background sweep that fills an existing library in (`docs/covers_plan.md`). The melonDS and Azahar Kotlin modules are still stubs (they hash ROMs for real, but every save call is a no-op). Haptics are done: the gamepad ticks on button presses via `expo-haptics`, behind a toggle on the Settings screen persisted in the SQLite `settings` table (`SettingsProvider` in `packages/ui`). The Home header carries two buttons: an info icon that opens `HelpScreen` and, to its right, the gear that opens Settings — a short, fully translated guide to importing ROMs (file or folder picker, the "Open with" path, and what import does to the file). Settings → Legal → License opens an in-app `LicenseScreen` showing that app's `licenseNotice`; the Terms of Use and Privacy Policy rows beside it open the browser at `AppConfig.termsUrl`/`privacyUrl` (set for gba, pointing at Google Sites pages; nds/threeds leave them unset so their rows stay inert until their pages are hosted). Translations are done: every UI string flows through i18next (`packages/ui/src/i18n`, ten bundled `.ts` catalogs — English default/fallback plus es-419, pt-BR, de, fr, it, ru, ja, ko, zh-Hans, matched to the device language by bare language code via `expo-localization`), with a Settings → General → Language override persisted in the `settings` table (`'auto'` follows the device); console names and the license notices deliberately stay English, and the Home header title translates through `home.title` around the untranslated console abbreviations from `ConsoleSpec`. Typography is done: two families ship in `packages/ui/assets/fonts/` and are embedded natively by an `expo-font` plugin entry in each `app.json` (no runtime load, no font flash) — Tourney Black is the display face on `typography.title`/`button` and the stack headers, Roboto 400/600 the reading face on `body`/`caption`; Tourney is Latin-only, so translated sentences must stay on the Roboto tier (`docs/app_font_plan.md`). Opening a ROM from outside the app works on `apps/gba`: `android.intentFilters` in its `app.json` puts it in Android's "Open with" list, `useOpenedRom` (`packages/ui`) picks the intent URI off React Native's `Linking`, and `importRomFromUri` (`packages/storage`) imports it — or recognises it by MD5 — before Home boots it. `apps/nds`/`apps/threeds` need only their own `app.json` filters; the JS half is already shared. Still to build: physical/Bluetooth controller support (needs key handling in the native views, which currently ignore all key events), battery-save import/export, 3DS SMDH icons (needs ExeFS decryption), and pad customization. iOS/store submission config is out of scope.
