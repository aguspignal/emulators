...
- `app.json` → `android.intentFilters` puts the app in Android's "Open with" list for `.nds` files. Same four-filter shape as gba's — see that app's CLAUDE.md for why both `content://` filters (with and without `mimeType`) are needed and why the `file://` `pathPattern`s are double-escaped. Editing this file needs a `prebuild` before the manifest changes take effect.
...
- `modules/melonds-core/` — local Expo Module (Android/Kotlin/C++), mirroring gba's `mgba-core`:
  ...
  - `android/.../MelondsCoreModule.kt` — implements the `EmulatorCore` contract and owns the idle/running/paused state machine + events, with the same `OnActivityEntersBackground`/`OnActivityEntersForeground`/`OnDestroy` hooks gba uses. `console` is a fixed `"nds"` — one console per app, so there is no header sniffing.
