...
- `modules/mgba-core/` — local Expo Module (Android/Kotlin/C++):
  ...
  - `android/.../MgbaCoreModule.kt` — implements the `EmulatorCore` contract and owns the idle/running/paused state machine + events. Its `OnActivityEntersBackground`/`OnActivityEntersForeground` hooks auto-pause a running game (and resume it) around backgrounding and screen-off — the emu thread and Oboe stream are native, so JS-side `AppState` handling alone can't stop them. `MgbaCoreNative.kt` is the JNI mirror (also maps contract button names → mGBA key bits). `RomFiles.kt` resolves ROM URIs (copies `content://` into cache) and keys battery saves/savestates by ROM SHA-1 under `filesDir/mgba/`. `MgbaCoreView.kt` hosts the SurfaceView, aspect-fits it, and hands the Surface to the engine.
