...
- Encrypted retail ROMs boot with **no user key dumps** (Azahar's built-in AES keyblob + open-source system archives). `.cia` is install-only upstream and out of scope here.
